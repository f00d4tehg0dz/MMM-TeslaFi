...
Module.register('MMM-TeslaFi',{
	...
	getDom: function() {
	 ...
	 const idealRange = t.ideal_battery_range ? (!this.config.imperial ?
 (t.ideal_battery_range * 1.0).toFixed(0) :
 (t.ideal_battery_range / 1.609).toFixed(0)) : 0;

	 const estRange = t.est_battery_range ? (!this.config.imperial ?
 (t.est_battery_range * 1.0).toFixed(0) :
 (t.est_battery_range / 1.609).toFixed(0)) : 0;
	 ...
	 const speed = t.speed ? (!this.config.imperial ?
 (t.speed * 1.0).toFixed(1) :
 (t.speed / 1.609).toFixed(1)) : 0;
	 const outside_temp = t.outside_temp ? (!this.config.imperial ?
 (t.outside_temp * 1.0).toFixed(1) :
 (t.outside_temp * 9 / 5 + 32).toFixed(1)) : 0;
	 const inside_temp = t.inside_temp ? (!this.config.imperial ?
 (t.inside_temp * 1.0).toFixed(1) :
 (t.inside_temp * 9 / 5 + 32).toFixed(1)) : 0;

	 const odometer = t.odometer ? (!this.config.imperial ?
		 (t.odometer * 1.0).toFixed(1) :
		 (t.odometer / 1.609).toFixed(0)) : 0;
